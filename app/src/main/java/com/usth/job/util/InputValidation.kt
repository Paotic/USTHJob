...
class InputValidation {
    companion object {
        ...
        fun isSapIdValid(sapId: String): Pair<Boolean, String> {
            if (sapId.isEmpty()) {
                return Pair(false, "ID cannot be empty.")
            }
            if (sapId.length != 12) {
                return Pair(false, "ID must be 12 characters.")
            }
            if (!sapId.matches("^[0-9]+$".toRegex())) {
                return Pair(false, "ID can only contain digits.")
            }
            return Pair(true, "")
        }
         ...
         *
         * @param dob the date of birth to validate in the format dd-MM-yyyy
        ...
        fun isDOBValid(dob: String): Pair<Boolean, String> {
            ...
            val dobDate = LocalDate.parse(dob, DateTimeFormatter.ofPattern("dd-MM-yyyy"))
